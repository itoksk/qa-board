# Opinion Board - 教育イベント意見集約システム

教育関連のイベントで参加者から集めた意見を地域・カテゴリ別に整理し、AIを活用して代表的な意見を自動生成するシステムです。

## 概要

このシステムは、大規模な教育イベントで寄せられる多数の意見を効率的に管理・分析するために開発されました。Google Apps Script (GAS) で構築され、Google Sheetsをデータベースとして使用します。
...
- 📁 **カテゴリ分類**: 
  - NotebookLMが活躍しような場面
  - Gemを使ってみた感想や期待感
  - 生成AIを手にした私たちは、子どもたちのためにどんな新しい教育をデザインできるでしょうか？
- 🤖 **AI要約**: TF-IDFとK-Meansクラスタリングで類似意見を自動分析し、Gemini APIで代表意見を生成
- 🌐 **カテゴリ横断クラスタリング**: 異なるカテゴリの類似意見も統合可能
- 👍 **いいね機能**: 参加者が共感する意見に「いいね」を付けられる（絵文字アイコン対応）
- 🔐 **管理者機能**: パスワード保護された管理画面
- 📊 **データ永続化**: Google Sheetsでデータを管理
- 📝 **簡潔で的確な代表意見**: 50-80文字程度で意見群の意図を網羅
...
1. `gas/`フォルダ内のすべての`.gs`ファイルをGoogle Apps Scriptプロジェクトにコピー
   ...
   - `QuestionClusterer.gs` - 意見クラスタリング
   - `GeminiService.gs` - Gemini API連携
...
```
参加者入力 → Google Sheets → クラスタリング処理 → Gemini API → 代表意見生成
```
...
4. **代表意見生成**: クラスタ内の意見を分析し、Gemini APIで自然な代表意見を生成

### 最新の改善点（2025年1月）

- **カテゴリ横断クラスタリング**: 異なるカテゴリに分類された類似意見も統合
- **適切な長さの代表意見**: 50-80文字程度で簡潔かつ的確に要約
- **意図を重視した生成**: クラスタ内の全意見の核心的な意図を確実に反映
- **自然な意見生成**: カテゴリ名に依存せず、意見内容に基づいた生成
- **完結性の保証**: すべての代表意見が必ず「？」で終わる完全な文として生成
...
```javascript
{
  id: string,           // UUID
  region: string,       // 地域コード
  question: string,     // 生成された代表意見
  category: string,     // カテゴリ
  clusterSize: number,  // クラスタ内の意見数
  sourceIds: string[],  // 元意見のID配列
...
| 代表意見が省略される | 最新版で修正済み、強制再生成を実行 |

詳細なトラブルシューティングは[使用ガイド](docs/USAGE_GUIDE.md#よくある意見と回答)を参照してください。
...
- `analyzeQuestionData()` - 意見データ分析
- `addSimilarTestQuestions()` - テストデータ追加
- `checkRepresentativeQuestionLength()` - 代表意見の文字数確認
...
### 2025年7月18日
- 代表意見の長さを最適化（50-80文字程度）
- 意見群の意図をより確実に反映するよう改善
